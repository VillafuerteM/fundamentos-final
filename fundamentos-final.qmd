...
l(p_1,p_2,p_3,p_4) = \\sum\_{i=1}\^{4}x\_{i}log(p\_{i})

\$\$

También sabemos que el estimador de máxima verosimilitud para cada \$p\_{i}\$ es \$\\frac{x\_{i}}{n}\$
...
**Rechazo el 7% de las veces**. El ***Error del tipo I*** es la probabilidad de rechazar hipótesis nula de \$H\_{0}\$ cuando es cierta, es decir que únicamente debemos calcular la distancia de aquellas simulaciones donde rechazamos

```{r fig.cap = "Distancia entre el error del tipo I y 5%"}
