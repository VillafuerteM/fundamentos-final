...
tabla_aux <- dplyr::left_join(very_liberal, 
                              poblacion_encuestas, 
                              by = "state")
...
```{r}
#| warning=FALSE, 
#| fig.cap= "Pct Very Liberal vs. Num. Encuestas (archivo DTA)",
#| echo=FALSE,
#| message=FALSE
...
**Respuesta:** Realizamos la unión de basos de datos
...
```{r}
#| warning=FALSE, 
#| fig.cap = "MLE vs. Pct de votantes por Obama",
#| echo=FALSE
...
**Respuesta:** Al tener un modelo beta-binomial sabemos que la **posterior tendrá una distribución Beta**. Previo a establecer una función general para la posterior realizamos simulaciones de la inicial para ver que información previa tenemos.
...
```{r echo=FALSE, fig.cap = "Distribución a priori: Beta (8,160)"}
ggplot(sim_inicial) + 
  geom_histogram(aes(x = theta, 
                     y = ..density..),
                 bins = 15, 
...
**Respuesta:** Realizamos estimaciones vía *Stan*. Dado que es la misma inicial un mismo modelo nos funciona para ambos Estados.
...
**Respuesta:** Realizamos las mismas gráficas; sin embargo, ahora utilizamos la media posterior como estimadores puntuales.
...
ggplot(tabla_aux_plot, aes(x = encuestas, 
                           y = value, 
                           color = Metodologia)) + 
  geom_point() +
  geom_text_repel(aes(label = state), 
  ...
  geom_smooth(method = "lm", 
              se = FALSE, 
              aes(group = Metodologia, 
                  color = Metodologia), 
              linetype = "dashed") +
  labs(title = "Num. Encuestas vs. Pct Very Liberal", 
       subtitle = "MLE vs. Bayesiano") + 
  xlab("Num. Encuestas") + 
  ylab("Pct Very Liberal") + 
  ylim(0,12) +
  ggpubr::theme_pubclean(base_size = 12)
```
...
ggplot(tabla_aux_plot, aes(x = vote_Obama_pct, 
                           y = value, color = Metodologia)) + 
  geom_point() +
  geom_text_repel(aes(label = state), 
                  box.padding = 0.5, 
                  size = 1.5) +
...
Todo el código generado para contestar el examen está disponible [aquí](https://github.com/VillafuerteM/fundamentos-final). Si hubiera dudas de la generación de algún gráfico o metodología, el código está abierto para consulta.
